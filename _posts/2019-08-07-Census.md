...
adult %>% 
  ...
  arrange(desc(p_na)) %>% 
  filter(q_na > 0) %>% 
  knitr::kable(caption = "NA's", format = "markdown") %>% 
  kable_styling()
{% endhighlight %}
...
adult %>% 
  ...
  knitr::kable(format = "markdown", caption = "Estadistica Descriptiva") %>% 
...
adult %>% 
  ...
  # select(1,3,4,6,5,7,2) %>% 
  knitr::kable(format = "markdown", caption = "Descriptius Outliers") %>% 
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"), full_width = FALSE)
{% endhighlight %}
